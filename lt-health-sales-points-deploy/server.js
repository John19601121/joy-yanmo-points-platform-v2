...
const SESSION_SECRET = process.env.SESSION_SECRET || "dev-change-me-lt-health";
...
if (process.env.NODE_ENV === "production" && SESSION_SECRET === "dev-change-me-lt-health") {
...
<head>
  ...
  <title>${escapeHtml(title)}｜LT大健康成交會員積分管理平台</title>
  <style>
...
<body>
  <div class="${user ? "shell" : ""}">
    ${user ? `<aside class="side"><div class="brand"><img src="/public/logo.png" alt="LT大健康成交 Logo"><div><b>LT大健康成交</b><span>會員積分管理平台</span></div></div>${nav(user)}</aside>` : ""}
    <main class="${user ? "main" : ""}">${user ? `<div class="top"><div><div class="kicker">LT HEALTH MEMBER POINTS</div><h1>${escapeHtml(title)}</h1></div><div class="user">${escapeHtml(user.name)}・${escapeHtml(user.role)}</div></div>` : ""}${content}</main>
  </div>
</body>
</html>`;
}
...
function loginPage(role, error = "", slug = "") {
  ...
  return page(title, `<div class="login">
    <section class="login-card">
      <div class="brand"><img src="/public/logo.png" alt="LT大健康成交 Logo"><div><b>LT大健康成交</b><span>會員積分管理平台</span></div></div>
      <h1>${title}</h1>
...
function handleExport(req, res, pathname) {
  if (pathname.startsWith("/admin/export/")) {
    ...
    if (pathname.endsWith("/all.xlsx")) {
      sendXlsx(res, "lt-health-all-report.xlsx", [
    ...
    if (pathname.endsWith("/stores.csv")) sendCsv(res, "lt-health-stores.csv", storeReportRows());
    else if (pathname.endsWith("/members.csv")) sendCsv(res, "lt-health-members.csv", memberReportRows());
    else if (pathname.endsWith("/transactions.csv")) sendCsv(res, "lt-health-transactions.csv", transactionReportRows());
    else if (pathname.endsWith("/requests.csv")) sendCsv(res, "lt-health-requests.csv", requestReportRows());
    else return false;
    return true;
  }
  if (pathname.startsWith("/store/export/")) {
    ...
    if (pathname.endsWith("/all.xlsx")) {
      sendXlsx(res, "lt-health-store-report.xlsx", [
    ...
    if (pathname.endsWith("/members.csv")) sendCsv(res, "lt-health-store-members.csv", memberReportRows(user.store_id));
    else if (pathname.endsWith("/transactions.csv")) sendCsv(res, "lt-health-store-transactions.csv", transactionReportRows(user.store_id));
    else if (pathname.endsWith("/requests.csv")) sendCsv(res, "lt-health-store-requests.csv", requestReportRows(user.store_id));
    else return false;
    return true;
  }
  return false;
...
http.createServer(router).listen(PORT, HOST, () => {
  console.log(`LT大健康成交會員積分管理平台 running at http://${HOST}:${PORT}`);
});
