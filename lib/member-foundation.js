...
function createActivationToken(db, memberId, ttlMinutes = 60 * 24) {
  db.exec("BEGIN IMMEDIATE");
  try {
    const token = issueActivationToken(db, memberId, ttlMinutes);
    db.exec("COMMIT");
    return token;
  } catch (error) {
    db.exec("ROLLBACK");
...
function issueActivationToken(db, memberId, ttlMinutes = 60 * 24) {
...
function registerPendingMember(db, {
  ...
  try {
    ...
    const protection = sharingFoundation.activeProtectionByIdentity(db, {
      email: normalizedEmail,
      phone: normalizedPhone,
      now
    });
    const codeReferrer = referralCode
      ...
      : null;
    if (referralCode && !codeReferrer && !protection) throw new Error("Referrer is invalid.");
    const referrer = protection
      ? { id: protection.protected_by_member_id }
      : codeReferrer;
    ...
    if (referrer) {
      db.prepare(`INSERT INTO member_referrals
        ...
        .run(
          member.id,
          referrer.id,
          protection ? "event_protection" : source,
          protection ? "converted from 30-day event protection" : "self registration"
        );
    }
    if (protection && !sharingFoundation.consumeProtection(db, protection.id, member.id, now)) {
      throw new Error("Event protection could not be converted.");
    }
    db.prepare(`INSERT INTO audit_events (event_type, subject_type, subject_id, metadata_json)
      ...
      .run(member.id, JSON.stringify({
        ...
        event_protection_id: protection?.id || null,
        supplied_referral_code_ignored: Boolean(protection && referralCode)
      }));
    const activationToken = issueActivationToken(db, member.id, activationTtlMinutes);
    db.exec("COMMIT");
    return {
      memberId: member.id,
      userId: user.id,
...
function pendingMemberByEmail(db, email) {
  const normalizedEmail = normalizeEmail(email);
  if (!normalizedEmail) return null;
  return db.prepare(`SELECT members.id AS memberId, members.name, members.email
    FROM members
    JOIN member_profiles ON member_profiles.member_id = members.id
    WHERE member_profiles.normalized_email = ?
      AND member_profiles.activation_status = 'pending'
    LIMIT 1`).get(normalizedEmail) || null;
}

function recordActivationEmailAudit(db, {
  eventType,
  memberId = null,
  email = null,
  ip = null,
  ...
  db.prepare(`INSERT INTO audit_events (event_type, subject_type, subject_id, metadata_json)
    VALUES (?, 'member', ?, ?)`).run(eventType, memberId, JSON.stringify(metadata));
}

function activationEmailAttemptCount(db, { eventTypes, email = null, ip = null, sinceMinutes = 60 }) {
  const types = Array.isArray(eventTypes) ? eventTypes : [eventTypes];
  const placeholders = types.map(() => "?").join(", ");
  const filters = [`event_type IN (${placeholders})`, "created_at >= datetime('now', ?)"];
  const params = [...types, `-${Number(sinceMinutes)} minutes`];
  if (email) {
    filters.push("json_extract(metadata_json, '$.email') = ?");
    params.push(normalizeEmail(email));
  }
  if (ip) {
    filters.push("json_extract(metadata_json, '$.ip') = ?");
    params.push(String(ip));
  }
  return db.prepare(`SELECT COUNT(*) AS count FROM audit_events WHERE ${filters.join(" AND ")}`).get(...params).count;
}
...
module.exports = {
  ...
  pendingMemberByEmail,
  recordActivationEmailAudit,
  activationEmailAttemptCount,
  setReferral
};
