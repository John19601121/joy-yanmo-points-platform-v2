...
function registerPendingMember(db, {
  ...
  try {
    ...
    db.exec("COMMIT");
    return { memberId: member.id, userId: user.id, memberCode: code, activationToken };
  } catch (error) {
    db.exec("ROLLBACK");
    throw error;
  }
...
function setReferral(db, memberId, referrerMemberId, source, actorUserId = null, reason = null) {
  ...
  try {
    ...
    const previous = db.prepare(`SELECT id, referrer_member_id
      FROM member_referrals WHERE member_id = ? AND status = 'active'`).get(memberId) || null;
    if (previous?.referrer_member_id === referrerMemberId) {
      db.exec("COMMIT");
      return;
    }
    ...
    db.prepare(`INSERT INTO audit_events
      ...
      VALUES ('member_referral_changed', ?, 'member', ?, ?)`)
      .run(actorUserId, memberId, JSON.stringify({
        previous_referrer_member_id: previous?.referrer_member_id || null,
