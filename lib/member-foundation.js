...
function activateMember(db, token) {
  ...
  const record = db.prepare(`SELECT id, member_id, expires_at, used_at FROM member_activation_tokens
    WHERE token_hash = ?`).get(tokenHash);
  if (!record || record.used_at || Date.parse(record.expires_at) <= Date.now()) throw new Error("Activation token is invalid or expired.");
  db.exec("BEGIN IMMEDIATE");
  try {
    const result = db.prepare(`UPDATE member_activation_tokens SET used_at = CURRENT_TIMESTAMP
      WHERE id = ? AND used_at IS NULL`).run(record.id);
    if (result.changes !== 1) throw new Error("Activation token was already used.");
    db.prepare("UPDATE member_profiles SET activation_status = 'active', updated_at = CURRENT_TIMESTAMP WHERE member_id = ?").run(record.member_id);
