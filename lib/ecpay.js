...
function paymentConfigForMerchantId(merchantId, environment = process.env) {
  ...
  const active = paymentConfig(environment);
  return active.merchantId === requestedMerchantId ? active : null;
}
...
function assertCallbackAllowed(config) {
  if (config.mode === "stage" && config.stageEnabled) return;
  if (config.mode === "production" && config.productionEnabled && config.productionMerchantApproved) return;
  throw new Error("ECPay callback environment is not configured or approved.");
}
